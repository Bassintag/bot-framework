...
export class ContainerManager implements IContainerManager {
    ...
    constructor(
        private readonly $config: IContainerOptions,
    ) {
    ...
    bind(): void {
        ...
        for (const resource of resources) {
            ...
            this.$container.bind<Repository<any>>(IDENTIFIERS.REPOSITORY)
                .toConstantValue(repository)
                .whenTargetNamed(resource.name);
            if (resource.service) {
                this.$container.bind<IResourceService<any>>(IDENTIFIERS.RESOURCE_SERVICE)
                    .to(resource.service)
                    .inSingletonScope()
                    .whenTargetNamed(resource.name);
            }
        ...
        // Bind events
        this.$container.bind<IEventManager>(IDENTIFIERS.EVENT_MANAGER)
        ...
        this.$container.bind<ISettingsService>(IDENTIFIERS.SETTINGS_SERVICE)
            .to(SettingsService).inSingletonScope();
        this.$container.bind<IWindowService>(IDENTIFIERS.WINDOW_SERVICE)
            .to(WindowService).inSingletonScope();

        // Migrations
        const migrations = this.$config.migrations || [];
        migrations.push(BaseMigration);
        for (const migration of migrations) {
            this.$container.bind<IMigration>(IDENTIFIERS.MIGRATION)
                .to(migration).inSingletonScope();
        }
        this.$container.bind<IMigrationManager>(IDENTIFIERS.MIGRATION_MANAGER)
            .to(MigrationManager).inSingletonScope();
        ...
        for (const taskType of taskTypes) {
            ...
            if (service != null) {
                this.$container.bind<ITaskService<any>>(IDENTIFIERS.TASK_SERVICE)
                    .to(service).inSingletonScope().whenTargetNamed(taskType.name);
            }
            this.$container.bind<ITask>(IDENTIFIERS.TASK_PROCESS).to(taskType.class)
                .whenTargetNamed(taskType.name);
            this.$container.bind<Factory<ITask>>(IDENTIFIERS.TASK_PROCESS_FACTORY).toFactory<ITask>((context) => {
