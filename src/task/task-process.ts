...
export abstract class TaskProcess<T, ResultT = any, ContextT = any> implements ITask<T, ResultT> {
	...
	private async handleMessage(request: IProcessRequest<any>): Promise<IProcessResponse<any>> {
		...
		return new Promise((resolve) => {
			...
			if (obs != null) {
				obs.pipe(takeUntil(this.$stopped), take(1)).subscribe(
					(result) => {
						resolve({
							channel,
							response: {
	...
	public async run(): Promise<ResultT> {
		...
		fromProcessEvent<IProcessRequest<any>>(this.process, 'message')
			.pipe(
		...
		return new Promise<ResultT>((resolve, reject) => {
			...
			fromProcessEvent(this.process!, 'exit')
				.pipe(takeUntil(this.$stopped))
				.subscribe(() => error());
			fromProcessEvent(this.process!, 'error')
				.pipe(takeUntil(this.$stopped))
				.subscribe(() => error());
		});
	}
